...
8. **`Before running the following playbooks `certbot` needs to be manually run`**
    ...
    ```
9.  playbook-nginx-add-domain.yml
    ...
    2.  Requires the use of `--limit` to make sure this runs for __only one__ server/host at a time
        `--limit <APOLLO-FQDN>`
10. **`Before running the following playbooks `certbot` needs to be manually run again`**
...
12. **`Wait 3mins to allow apollo database tables to be created`**
13. playbook-apollo-docker-postgres-create-admin.yml
