...
/usr/bin/rsync -e ssh -avrL --numeric-ids --rsync-path="sudo rsync" backup_user@$REMOTE_HOST:/etc/ $BACKUP_DIR --log-file=$LOGFILE
/usr/bin/rsync -e ssh -avrL --numeric-ids --rsync-path="sudo rsync" backup_user@$REMOTE_HOST:/var/www/ $BACKUP_DIR --log-file=$LOGFILE
