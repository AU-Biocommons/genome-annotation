...
USAGE_STR="Usage: $(basename $0) [-h(elp)] [-v(erbose)] [-t(est)] [-r(ecurse)] [-p(ath) swift_install_dir] [-c(reds) openstack_cred_file] [-m(time) days] [-f(files) file_regex] [-d(irs) dir_regex] [-b(ackup) root_dir] container"
...
Where:
    ...
    -r                  recurse into subdirectories for files matching regex (default search top level only)
...
while getopts "b:d:f:m:c:p:trvh" opt; do
    case $opt in
        h)
            show_help
            exit 0
        ...
        t)
            ...
            ;;
        r)
            SEARCH_DEPTH="" # no maxdepth for find
            ;;
        p)
...
# search for matching files in the directory specified (does NOT recurse into subdirectories)
[ $VERBOSE_MODE -eq 1 ] && echo "find ${BACKUP_ROOT}/${DIR_REGEX} ${SEARCH_DEPTH} -name \"${FILE_REGEX}\" -mtime ${MTIME} -print"
ARCHIVE="$(find ${BACKUP_ROOT}/${DIR_REGEX} ${SEARCH_DEPTH} -name "${FILE_REGEX}" -mtime ${MTIME} -print)"
#WEEKLY_ARCHIVES="$(find $BACKUP_ROOT/apollo-???_archive/ ${SEARCH_DEPTH} -name "apollo-???_*.tgz" -mtime -7 -print)"
if [ $? -ne 0 ]; then
